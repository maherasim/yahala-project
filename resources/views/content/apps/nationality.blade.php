                ...
                <tbody>
                    @foreach ($nationality as $key => $nationality)
                        <tr>
                            ...
                            <td><img style="border-radius: 100%; height: 43px; width: 43px;" src="{{ asset('storage/' . $nationality->thumbnail_path) }}" alt="{{ $nationality->name }}"></td>
                            <td>
                                <a href="javascript:;" class="btn btn-sm btn-icon edit-button" data-bs-toggle="modal" data-bs-target="#addCategory" data-id="{{ $nationality->id }}" data-name="{{ $nationality->name }}"><i class="bx bxs-edit"></i></a>
                               
                                <form action="{{ route('nationality.destroy', $nationality->id) }}" onsubmit="confirmAction(event, () => event.target.submit())" method="post" class="d-inline">
                                    @method('DELETE')
                                    @csrf
...
<div class="modal fade" id="addCategory" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('nationality.store') }}" enctype="multipart/form-data" method="POST">
                @csrf
